...
export default function DashboardPage() {
  ...
  const myAssignedMatches = matches.filter((m) => m.refereeAssignment?.referee.id === user.id).length;
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted">Here&apos;s what&apos;s happening across your tournaments.</p>
      </div>

      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard label="Ongoing tournaments" value={ongoing} icon={Trophy} accent="brand" />
        <StatCard label="Total tournaments" value={tournaments.length} icon={Trophy} accent="slate" />
        ...
        <StatCard label="Upcoming matches" value={matches.length} icon={CalendarClock} accent="brand" />
      </div>
