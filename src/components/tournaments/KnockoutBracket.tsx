            ...
            <div className="flex h-full flex-col justify-around">
              {round.matches.map((match) => (
                <div key={match.id} className="relative py-2">
                  ...
                  {roundIdx > 0 && (
                    <span className="absolute left-[-2.5rem] top-1/2 h-px w-10 bg-border" />
                  )}
                </div>
              ))}
            </div>
  ...
  return (
    ...
    >
      <BracketTeamRow
        ...
        isWinner={!!winnerId && winnerId === match.homeTeam?.id}
      />
      <div className="border-t border-border" />
      <BracketTeamRow
        team={match.awayTeam}
        score={match.awayScore}
        isWinner={!!winnerId && winnerId === match.awayTeam?.id}
      />
...
function BracketTeamRow({
  ...
  return (
    <div
      className={`flex items-center justify-between gap-2 px-3 py-2 text-sm ${isWinner ? "font-semibold text-foreground" : "text-muted"}`}
    >
