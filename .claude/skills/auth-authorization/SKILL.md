...
description: Auth and RBAC conventions for STMS — use when touching src/lib/auth.ts, middleware.ts, rbac.ts, or any protected route/page.
---
...
- `middleware.ts` protects page routes by path matcher; every API route independently re-checks auth + role too (defense in depth — never rely on UI hiding or middleware alone).
